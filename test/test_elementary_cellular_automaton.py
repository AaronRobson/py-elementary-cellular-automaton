...
class TestRulesSettings(unittest.TestCase):
    ...
    def testSettingsExpected(self):
        ...
        self.assertEqual(len(tuple(self.s.neighhbourHoodConfigurations)), self.s.numNeighhbourHoodConfigurations)

        self.assertEqual(len(tuple(self.s.wolframCodes)), self.s.numOfWolframCodes)
...
class TestRules(unittest.TestCase):
    ...
    def testBoolCollectionToBase2Str(self):
        self.assertEqual(eca.BoolCollectionToBase2Str((True,False,None)), '100')
    ...
    def testR30(self):
        self.assertEqual(tuple(map(self.r30, self.s.neighhbourHoods)), (False,False,False,True,True,True,True,False))

    def testNextLineSpecifyRule(self):
        self.assertEqual(tuple(eca.NextLineSpecifyRule(self.r30, (True,True,True))), (True,True,False,False,True))
    ...
    def testRuleGeneratorFullPadded(self):
        arrangements = (
            ...
            ((None, None, True, None, None), (None, True, True, True, None), (True, True, False, False, True)),
        )

        values = islice(eca.RuleGeneratorArrangementsPadded(self.r30), 3)
        values = tuple(tuple(tuple(line) for line in arrangement) for arrangement in values)

        self.assertEqual(arrangements, values)

    def testRuleGeneratorFullPaddedString(self):
        self.assertEqual(tuple(islice(eca.RuleGeneratorArrangementsPaddedStrings(self.r30), 3)), ('1', ' 1 \n111', '  1  \n 111 \n11001'))
